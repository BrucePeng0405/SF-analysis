...
tmp <- subset(all,idents = c("Migration Myofibroblasts"))
Idents(tmp) <- "group"
x <- AverageExpression(tmp)
exp1 <- as.matrix(x[["RNA"]])

#添加Celltype_Group作为新的matadata，并且设为idents
...
#load(file = "../../mast/10x_RawExpressionMatrix.rdata")



#按cluster进行GSVA分析及可视化
es.max <- gsva(exp1, fgsea_set, mx.diff = F, kcdf = "Poisson")
write.csv(es.max, "mast_GSVA_Migration Myofibroblasts_results.csv")

rd1 <- read.csv(file = "mast_GSVA_Adhesion Fibroblasts_results.csv")
rd2 <- read.csv(file = "mast_GSVA_Adhesion Myofibroblasts_results.csv")
rd3 <- read.csv(file = "mast_GSVA_Cycle Myofibroblasts_results.csv")
rd4 <- read.csv(file = "mast_GSVA_Inflammatory Fibroblasts_results.csv")
rd5 <- read.csv(file = "mast_GSVA_Migration Myofibroblasts_results.csv")
library(dplyr)
sig_markers <- inner_join(rd1,rd2, by = "X")
sig_markers <- inner_join(sig_markers,rd3, by = "X")
sig_markers <- inner_join(sig_markers,rd4, by = "X")
sig_markers <- inner_join(sig_markers,rd5, by = "X")
write.csv(sig_markers, "mast_GSVA_all_results.csv")
...
sig_markers <- read.csv("selected_GSVA_Adhesion Fibroblasts_results.csv")
