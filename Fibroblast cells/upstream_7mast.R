...
#==================================================================================================
mast_df <- read.csv("mast/mast_df.csv")
df <- mast_df
...
# Adjust factors
CELLS <- c("Migration Myofibroblast","Adhesion Myofibroblast",
...
q <- ggplot(df, aes(x = celltype_factor, y = z, color = col)) +
  ...
  theme(axis.text.x = element_text(angle=15, hjust=1, size = 14), axis.title.x = element_blank()) +
...
q
ggsave(paste0("mast/group.dependent.degs", Sys.Date(), ".pdf"), height = 5, width = 8.5)
dev.off()
...
tmp <- data.frame(Cell_type="Antigen-presenting Fibroblast",orig.ident=c("A_7D"),count=0)
...
enrich_df <- read.csv("mast/ProportionEnrichments.csv")
library(plyr)
...
p <- ggplot(enrich_df, aes(x = celltype_factor, y = Log2Enrich)) +
  ...
  geom_hline(aes(yintercept=0), color="darkgrey", linetype="dashed")
p
ggsave(paste0("mast/group.enrich", Sys.Date(), ".pdf"), height = 5, width = 8.5)
dev.off()
