...
all <- subset(all, subset = orig.ident %in% c("A_2M"))
...
ldat.A14D <- read.loom.matrices("loom_file/cellsorted_A_2M_name_sorted_5AEI2.loom")
emat.A14D <- ldat.A14D$spliced
colnames(emat.A14D) <- gsub("cellsorted_A_2M_name_sorted_5AEI2:", "A_2M_", colnames(emat.A14D))
...
colnames(nmat.A14D) <- gsub("cellsorted_A_2M_name_sorted_5AEI2:", "A_2M_", colnames(nmat.A14D))
...
save(rvel,file = "velocity_all/A_2M_rvel.Rdata")
...
#pca降维并绘图
pdf(file = "velocity_all/A_2M_velocity.pdf", width = 10, height = 10)
