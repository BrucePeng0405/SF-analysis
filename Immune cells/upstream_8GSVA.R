...
es.max <- read.csv("GSVA/avg_hallmark/GSVA_results.csv")
...
sig_markers <- read.csv("GSVA/avg_hallmark/selected_GSVA_results.csv")
...
sig_markers <- read.csv("GSVA/avg_hallmark/selected_GSVA_results-1.csv")
sig_markers$X <- gsub("GOBP_","",sig_markers$X)
sig_markers$X <- gsub("_"," ",sig_markers$X)

#自定义配色：
mycol2<- colorRamp2(c(-2, 0, 2), c("#AFC2D9", "white", "#C6307C"))
...
#cols<- c("#AA0DFE", "#3283FE", "#85660D", "#782AB6", "#565656", "#1C8356", 
#         "#16FF32", "#F7E1A0")
cols<- c("#435b95", "#afc2d9", "#89558d", "#c6307c", "#79b99d", "#d0afc4", 
         "#e69f84","#4991c1")


#cols<- c("#4991c1", "#afc2d9", "#c6307c", "#435b95", "#79b99d", "#e69f84", 
#         "#89558d","#d0afc4")
...
#热图绘制：
#aver_dtt <- aver_dtt[,c(4,2,7,3,5,8,6,1)]
pdf("GSVA/avg_hallmark/GSVA-2.pdf", width =16, height = 8)
Heatmap(aver_dtt,
        name= 'Pathway activity',
        col= mycol2,
        cluster_columns= F,
