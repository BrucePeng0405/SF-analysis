...
all <- subset(all, subset = orig.ident %in% c("A_7D","A_14D"))
...
ldat.A7D <- read.loom.matrices("loom_file/cellsorted_A_7D_name_sorted_QH0U0.loom")
emat.A7D <- ldat.A7D$spliced

colnames(emat.A7D) <- gsub("cellsorted_A_7D_name_sorted_QH0U0:", "A_7D_", colnames(emat.A7D))
colnames(emat.A7D) <- gsub("x", "", colnames(emat.A7D))

ldat.A14D <- read.loom.matrices("loom_file/cellsorted_A_14D_name_sorted_5J72C.loom")
emat.A14D <- ldat.A14D$spliced
colnames(emat.A14D) <- gsub("cellsorted_A_14D_name_sorted_5J72C:", "A_14D_", colnames(emat.A14D))
...
colnames(nmat.A7D) <- gsub("cellsorted_A_7D_name_sorted_QH0U0:", "A_7D_", colnames(nmat.A7D))
colnames(nmat.A7D) <- gsub("x", "", colnames(nmat.A7D))

nmat.A14D <- ldat.A14D$unspliced
colnames(nmat.A14D) <- gsub("cellsorted_A_14D_name_sorted_5J72C:", "A_14D_", colnames(nmat.A14D))
...
save(rvel,file = "velocity_all/A_7D_14D_rvel.Rdata")
...
#全局速率可视化
pdf(file = "velocity_all/A_7D_14D_velocity.pdf", width = 10, height = 10)
...
#pca降维并绘图
pdf(file = "velocity_all/A_14D_velocity_PCA.pdf", width = 10, height = 10)
